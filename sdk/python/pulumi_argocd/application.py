...
class ApplicationArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[_builtins.bool] cascade: Whether to applying cascading deletion when application is removed.
        :param pulumi.Input[_builtins.bool] validate: Whether to validate the application spec before creating or updating the application.
        :param pulumi.Input[_builtins.bool] wait: Upon application creation or update, wait for application health/sync status to be healthy/Synced, upon application deletion, wait for application to be removed, when set to true. Wait timeouts are controlled by the provider Create, Update and Delete resource timeouts (all default to 5 minutes). **Note**: if ArgoCD decides not to sync an application (e.g. because the project to which the application belongs has a `sync_window` applied) then you will experience an expected timeout event if `wait = true`.
        """
...
@pulumi.type_token("argocd:index/application:Application")
